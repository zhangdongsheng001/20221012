		...
		<div  v-for="(item,j) in indexItem"  class="experience-list" :key="`itemsaishi-${j}`" :ref="'itemsaishi'+j" >
			<template v-if="item.begin_time">
				...
				<div class="item flex-row align-center" >
				    ...
				    <input type="text" placeholder="请填写公司名称" v-model="item.name">
				</div>
				<div class="m2">
					<div>担任职位</div>
					<div class="input-box">
						<textarea rows="3"  placeholder="请填写担任职位"  @keyup="changeTextStr($event,j)" maxlength="30">
							{{item.position}}
						</textarea>
						<div class="num">
			...
			<template v-else>
				...
				<div class="item flex-row align-center m1" >
				    <div class="label flex1">结束时间</div>
					<p @click="changeDate(indexItem,'end_time',j)">
						<span>2021</span>年
						<span>05</span>月
						<span>01</span>日
					</p>
				...
				<div class="m2">
					<div>担任职位</div>
					<div class="input-box">
						<textarea rows="3"  placeholder="请填写担任职位"  @keyup="changeTextStr($event,'a')" maxlength="30">
