<template>
  <div class="content-box bgwhite mt-flex mt-flex-y">
    <div class="ui-header-item gradient">
		...
		<div class="ui-header-title">支付</div>
    ...
    <div v-else class="ui-mode-content mt-flex-inner mt-flex mt-flex-y bgGray pad-5" >
        <div class="mt-flex-inner mt-alipay-content">
            <div class="mt-alipay-box">
                ...
                <div class="mt-alipay-submit" @click="alipay()">支付宝支付</div>
            </div>
        </div>
        <!-- <div v-html="alipayhtml" id="alipayhtml"></div> -->
		<div  id="alipayhtml"></div>
    </div>
  </div>
</template>
 <script>
...
export default {
  ...
  methods: {
    ...
    alipay:function(e) {
    	...
    	if (!that.isWeixin) {
        this.$http.post({
			name:'common.shopping.pay.alipay', 
          data: {
	  ...
	  loading:{
          ...
          //that.alipayhtml=result.requestText
          //document.getElementById('alipaysubmit').submit()
		  document.getElementById('alipayhtml').innerHTML = result.requestText
