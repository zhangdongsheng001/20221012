...
class SelectCity {
    ...
    init(){
		...
		this.show();
		SelectCity.cache=SelectCity.cache||{};
		SelectCity.cache[this.id]=this;
		return this;
	}
	/*
	 * 获取默认数据
    ...
    remove(){
        if(this.selectInstance){
			...
			delete SelectCity.cache[this.id];
...
SelectCity.close=()=>{
	if(SelectCity.cache){
		Object.keys(SelectCity.cache).map(id=>{
	        SelectCity.cache[id]&&SelectCity.cache[id].close();
	    });
	};
};
