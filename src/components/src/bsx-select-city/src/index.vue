 ...
 */ -->
<template>
     <transition name="app_selectCity" v-on:after-leave="afterLeave">
			 <form ref="fromCity">
        <div class="app_selectCity-master" v-if="visible" @touchstart.prevent>
            <div class="app_selectCity-container" @click.stop>
                <!-- <div class="flex-row align-stretch app_selectCity-head">
				...
				<div class="app_selectCity-head">
				    <p class="flex1">{{title}}</p>
				</div>
				<div class="app_selectCity-body flex-row align-stretch">
						...
						<div v-else class="flex1 align-center flex-row app_selectCity-null m">没有数据</div>
				</div>
				<div class="app_selectCity-bottom flex-row"  >
        ...
        handleSure(){
           ...
           let items=this.$refs.item,res=[];
            ...
            if(this.__sure){
				...
				handleReset(){
					//this.$refs.item.city='';
					/* console.log(this.$refs.item.city)
					this.$refs['fromCity'].resetFields()
					console.log(this.$refs.item.city) */
					this.value=''
				},
        handleCancel(){
